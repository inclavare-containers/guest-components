...
use tokio_tar::{ArchiveBuilder, Entry};
use xattr::FileExt;
...
mod tests {
    ...
    async fn test_unpack() {
        if !nix::unistd::Uid::effective().is_root() {
            eprintln!("skipping whiteout ownership test: root privileges are required");
            return;
        }
        ...
        hdr.set_size(0);
        hdr.set_mode(0o644);
        hdr.set_uid(nix::unistd::Uid::effective().as_raw() as u64);
        hdr.set_gid(nix::unistd::Gid::effective().as_raw() as u64);
        ...
        let destination = &output.path().join("layer");
