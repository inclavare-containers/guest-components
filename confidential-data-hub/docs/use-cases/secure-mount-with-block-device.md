...
The SecureMount response keeps the downstream `mount_path` field and returns
the request's `mount_point` unchanged.

When `key` is a KBS resource URI, CDH continues to use the existing
OpenAnolis Trustee client and passport flow. These storage changes do not
introduce a dependency on the confidential-containers Trustee deployment.

### Legacy inclavare request compatibility

Existing callers do not have to switch request formats atomically. CDH also
accepts `volume_type: "BlockDevice"` and the historical options below:

| Legacy option | Upstream equivalent | Compatibility behavior |
| ------------- | ------------------- | ---------------------- |
| `encryptType: "LUKS"` | `encryptionType: "luks2"` | `LUKS` and `LUKS2` are accepted case-insensitively. |
| `encryptKey` | `key` | The original sealed-secret or KBS resource URI is preserved. |
| no `encryptKey` | `sourceType: "empty"` | CDH generates a random ephemeral key. |
| `encryptKey` present | `sourceType: "encrypted"` | CDH opens the existing encrypted device. |
| implicit filesystem target | `targetType: "fileSystem"`, `filesystemType: "ext4"` | The original ext4 mount behavior is retained. |
...
| `options.dataIntegrity`    | String (`"true"` / `"false"`)               | No (default `"false"`) | Enable (`"true"`) to turn on dm-integrity for stronger integrity guarantees; expect >30% IO performance overhead. Keep `"false"` for performance-sensitive ephemeral storage. |
...
For `sourceType: "empty"`, CDH creates a detached LUKS2 header under
`/run/confidential-containers/cdh/luks-headers`. Since `/run` is tmpfs in
the guest, the header disappears at reboot and the backing device alone cannot
be reopened. Use the manually prepared, on-device-header flow in the
persistent-storage example when the data must survive a reboot.

On older `cryptsetup` releases that cannot activate dm-integrity with a
detached header, CDH retries an empty-device initialization with an on-device
header. The operation remains ephemeral when no `key` was supplied because
the generated key is never persisted.
